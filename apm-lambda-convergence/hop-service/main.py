...
def invoke_lambda_proxy():
    ...
    try:
        ...
        print(f"HTTP error occurred calling API Gateway: {http_err}")
        try:
        ...
        print(f"A network error occurred trying to reach the API Gateway: {req_err}")
        return jsonify({"error": "A network error occurred trying to reach the API Gateway.", "details": str(req_err)}), 500
            
    except Exception as e:
        # Catch any other unexpected errors.
        print(f"An unexpected error occurred in the hop-service: {e}")
